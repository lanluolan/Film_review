...
class MyMessage_remark extends StatefulWidget{
  final String email;
  MyMessage_remark(this.email);
  @override
  MyMessage_remarkState createState() {
    return MyMessage_remarkState(email: email);
  }
}

class MyMessage_remarkState extends State<MyMessage_remark> with SingleTickerProviderStateMixin{
  MyMessage_remarkState({Key key,this.email}) : super();
  final String email;
  User userData;
  List remarks=[{}];
  ...
  GetUserInfo() async {
    ...
    var request = http.MultipartRequest('GET',
        Uri.parse('http://570ecf04.r3.cpolar.top/user_query?user_id=$email'));
  ...
  Future<void> getremarkList(String email) async {
    ...
    var request = http.Request('GET', Uri.parse('https://570ecf04.r3.cpolar.top/comment_query?user_id=$email'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      String responseContent=await response.stream.transform(utf8.decoder).join();
  ...
  Future<void> getmoviereviewList(String email,String movie_id) async {
    ...
    var request = http.Request('GET', Uri.parse('https://570ecf04.r3.cpolar.top/movie_reaction_query?user_id=$email&movie_id=$movie_id'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      String responseContent=await response.stream.transform(utf8.decoder).join();
  ...
  Future<void> getbookreviewList(String email,String book_id) async {
    ...
    var request = http.Request('GET', Uri.parse('https://570ecf04.r3.cpolar.top/book_reaction_query?user_id=$email&book_id=$book_id'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      String responseContent=await response.stream.transform(utf8.decoder).join();
  ...
  Future<void> getmovieList(String movie_id) async {
    ...
    var request = http.Request('GET', Uri.parse('https://570ecf04.r3.cpolar.top/movie_query?movie_id=$movie_id'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      String responseContent=await response.stream.transform(utf8.decoder).join();
  ...
  Future<void> getbookList(String book_id) async {
    ...
    var request = http.Request('GET', Uri.parse('https://570ecf04.r3.cpolar.top/book_query?book_id=$book_id'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      String responseContent=await response.stream.transform(utf8.decoder).join();
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Image.asset(
                              "images/person.jpg", width: rpx(80), height: rpx(80),),
                        ...
                        Text(value["content"], style: TextStyle(fontSize: rpx(16.0)),
                          ...
                          child: Row(
                            ...
                            children: [
                              Image.asset(
                                ...
                                width: rpx(100.0), height: rpx(90.0),),
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Image.asset(
                              "images/person.jpg", width: rpx(80.0), height: rpx(80),),
                            Padding(padding: EdgeInsets.only(left: rpx(15.0))),
                            Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
